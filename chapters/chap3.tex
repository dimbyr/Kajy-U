...
%\end{itemize}
\section{Exercices}
%\section{Exercices : Complexité algorithmique}
...
\begin{enumerate}
	...
	% \textbf{Réponses} :
	% \begin{enumerate}
		%     \item La complexité temporelle est `O(n)`, car chaque élément du tableau est parcouru une fois.
		%     \item La complexité temporelle est `O(n^3)`, car chaque entrée du résultat nécessite une somme de `n` produits.
		%     \item La complexité temporelle est `O(log n)`, car chaque étape divise l'espace de recherche par deux.
		% \end{enumerate}
	...
	% \textbf{Réponses} :
	% \begin{enumerate}
		%     \item La complexité temporelle est `O(n^2)`, car il y a deux boucles imbriquées, chacune avec `n` itérations.
		%     \item Pour réduire la complexité, vous pouvez utiliser des optimisations telles que l'élimination de boucles inutiles ou le recours à des techniques de pré-calcul.
		% \end{enumerate}
	...
	% \textbf{Réponses} :
	% \begin{enumerate}
		%     \item La complexité temporelle est `O(n)`, car chaque boucle parcourt le tableau une fois.
		%     \item Les deux boucles sont séquentielles, chacune effectuant `n` opérations. Le temps d'exécution est proportionnel à la taille du tableau.
		% \end{enumerate}
	...
	% \textbf{Réponses} :
	% \begin{enumerate}
		%     \item `O(n \log n)` signifie que le temps d'exécution augmente de manière logarithmique par rapport à la taille de l'entrée, généralement en raison de divisions successives.
		%     \item Un exemple est le tri rapide (QuickSort), qui divise l'espace de recherche avec des appels récursifs. Le `log n` vient du nombre de fois où le problème est divisé, tandis que `n` représente le nombre total d'opérations.
		% \end{enumerate}
	...
	% \textbf{Réponses} :
	% \begin{enumerate}
		%     \item `O(1)`, car l'opération prend un temps constant.
		%     \item `O(n)`, car chaque élément est additionné une fois.
		%     \item `O(n)`, car il faut parcourir toute la liste pour trouver l'élément.
		%     \item `O(n^2)`, car le tri par insertion utilise des boucles imbriquées avec un nombre quadratique d'opérations.
		% \end{enumerate}
\end{enumerate}
